...
namespace UcenikShuffle.Common
{
	public static class HelperMethods
	{
		...
		public static long GetCombinationCount(IEnumerable<int> groupSizes)
		{
			...
			*/
			...
			int availableNumberCount = sameSizeGroups.Select(g => g.Sum()).Sum();
		...
		private static long GetCombinationCountForGroup(int groupSize, int availableNumberCount)
		{
			#region Algorithm explanation
			...
			for (int j = 0; j < numberFrequencies.Count; j++)
			{
				...
				{
					numberOfCombinations += (j + 1) * numberFrequencies[j][k];
				}
			}

			return numberOfCombinations;
		}
		private static long GetCombinationCountForSameSizeGroups(IList<int> groupSizes, int availableNumberCount)
		{
			if (groupSizes.Count == 1)
			{
			...
			{
				long tempNumberOfCombinations = groupSize == 0 ? 1 : GetCombinationCountForGroup(groupSize, availableNumberCount - 1);
